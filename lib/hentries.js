...
function _handleNode(baseUrl, thisUrl, node, log, get, enqueue, callback) {
	...
	if (node.type[0] === 'h-feed') {
		...
		node.children.forEach(function(child) {
				...
				get(resolvedUrl, function(err, data) {
					...
					mf2.get({html: data}, function(err, data) {
						...
						data.items.forEach(function(root) {
							...
							_handleNode(baseUrl, resolvedUrl, root, log, get, noop, callback);
						});
					});
				});
			...
			*/
			...
			process.nextTick(_handleNode.bind(undefined, baseUrl, thisUrl, child, log, get, enqueue, callback));
		});
...
function makeHentriesHandler(_log) {
	...
	return function handleNode(baseUrl, node, callback) {
		...
		_handleNode(baseUrl, baseUrl, node, log, get, function() {
