...
function _handleNode(baseUrl, thisUrl, node, enqueue, callback) {
	...
	if (node.type[0] === 'h-feed') {
		node.children.forEach(function(child) {
				...
				get(resolvedUrl, function(err, data) {
					...
					mf2.get({html: data}, function(err, data) {
						...
						data.items.forEach(function(root) {
							...
							_handleNode(baseUrl, resolvedUrl, root, noop, callback);
						});
					});
				});
			...
			*/
			process.nextTick(_handleNode.bind(undefined, baseUrl, thisUrl, child, enqueue, callback));
		});
...
function handleNode(baseUrl, node, callback) {
	...
	_handleNode(baseUrl, baseUrl, node, function() {
		waitingFor++;
