...
var assert = require('assert'),
    ...
    uuid = require('uuid'),
    log = require('./log').child({component: 'persistence'});
...
function get(key, cb) {
	...
	log.debug({key: key, job_id: id}, 'Retrieving key.');

	fs.readFile(computePath(key), function(err, data) {
		// XXX make sure it was the leaf of the directory tree
		if (err && err.code === 'ENOENT') {
			log.debug({key: key, job_id: id}, 'Key does not exist; returning empty store.');
			cb(undefined, {});
			return;
		}

		if (err) {
			log.error({key: key, job_id: id, err: err}, 'Error retrieving key.');
			cb(err);
			return;
		}

		data = JSON.parse(data.toString());
		log.debug({key: key, job_id: id, value: data}, 'Retrieved key.');
		cb(undefined, data);
	});
}
...
function set(key, data, cb) {
	...
	log.debug({key: key, value: data, job_id: id}, 'Setting key.');

	writeFileAtomic(computePath(key), JSON.stringify(data), function(err) {
		if (err) {
			log.error({key: key, err: err, job_id: id}, 'Error setting key.');
		} else {
			log.debug({key: key, job_id: id}, 'Set key.');
		}
