	...
	return function sendWebmention(sourceUrl, updatedTimestamp, html, cb) {
		...
		db.get('PAGE', sourceUrl, function(err, data) {
			...
			links.each(function(idx, el) {
				...
				var target = URLToolkit.buildAbsoluteURL(sourceUrl, el.attribs.href);
				log.debug({target: target}, 'Attempting to send Webmention.');
				jobs++;
				webmention(sourceUrl, target, function(err, result) {
					if (err) {
						log.error({target: target, err: err}, 'Error sending Webmention.');
						cb(err);
						return;
					}
