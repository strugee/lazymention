...
var cheerio = require('cheerio'),
    ...
    _ = require('lodash'),
    uuid = require('uuid'),
    checkLocalhost = require('check-localhost'),
    url = require('url');

function _sendWebmention(log, db, sourceUrl, updatedTimestamp, html, cb) {
...
module.exports = function createWebmentionDispatcher(_log, db, allowLocalhost) {
	...
	return function sendWebmention(sourceUrl, updatedTimestamp, html, cb) {
		...
		if (!allowLocalhost) {
			checkLocalhost(hostname).then(function(isLocalhost) {
				console.log('fwja', isLocalhost, hostname);
				if (isLocalhost) {
					log.warn('Refusing to send Webmention because it\'s addressed to localhost.');
					cb();
					return;
				}

				_sendWebmention(log, db, sourceUrl, updatedTimestamp, html, cb);
			}).catch(function(err) {
				log.error({err: err}, 'Couldn\'t determine whether URL is on localhost; refusing to deliver.');
				cb();
			});
		} else {
			_sendWebmention(log, db, sourceUrl, updatedTimestamp, html, cb);
